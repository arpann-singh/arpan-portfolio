...
// Grammar fixed: "a UI/UX Designer" instead of "an"
const roles = [
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen relative w-full overflow-hidden bg-slate-100 pb-20 sm:pb-32">
      ...
      {/* HERO SECTION */}
      <section className="relative z-10 flex flex-col justify-center min-h-screen px-4 sm:px-6 md:px-10 lg:px-12 xl:px-16 container mx-auto pt-32 sm:pt-36 pb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6 auto-rows-fr max-w-7xl mx-auto w-full">
          ...
          <TiltCard
            ...
            className="col-span-1 md:col-span-2 lg:col-span-3 bg-white/60 backdrop-blur-3xl rounded-[2rem] p-6 sm:p-8 lg:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.06)] border-2 border-white/80 flex flex-col justify-center relative group cursor-default overflow-hidden h-full"
          >
            ...
            {/* Liquid Font Size + Wrap */}
            <h1 ref={heroTextRef} className="font-syne text-[2rem] leading-[1.1] sm:text-5xl md:text-5xl lg:text-[3.25rem] xl:text-6xl font-extrabold tracking-tighter mb-4 relative z-10 text-slate-900 pointer-events-none">
              <span className="block mb-1 sm:mb-2 text-slate-800">I'm</span>
              <div className="min-h-[2.5em] sm:min-h-[1.3em] flex items-center mt-1">
                <AnimatePresence mode="wait">
                  ...
                  >
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-600 pb-1 pr-2">
                      {roles[currentRole].text}
                    </span>
                    <span className="text-3xl sm:text-5xl md:text-5xl lg:text-5xl xl:text-6xl inline-block drop-shadow-sm pb-1">
          ...
          <TiltCard
            ...
            className="col-span-1 md:col-span-1 lg:col-span-1 lg:row-span-2 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.12)] border-4 border-white overflow-hidden relative min-h-[300px] sm:min-h-[350px] lg:min-h-0 h-full w-full group"
          >
          ...
          <TiltCard
            ...
            className="col-span-1 md:col-span-1 lg:col-span-1 relative min-h-[200px] sm:min-h-[220px] lg:min-h-0 h-full w-full"
          >
            <div className="w-full h-full"><ApertureWidget /></div>
          </TiltCard>
          ...
          >
            ...
            <div>
              ...
              <h3 className="font-syne text-2xl sm:text-3xl font-bold text-slate-900 mb-1 tracking-tight">{timeString}</h3>
              <p className="text-slate-600 font-sans text-xs sm:text-sm font-bold">{statusText}</p>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-200/60 flex items-center gap-3 relative z-10 pointer-events-none">
              ...
              <div className="flex flex-col overflow-hidden flex-1">
                <div className="flex items-center gap-1.5 mb-0.5">
                  ...
                  <span className={`text-[9px] font-bold uppercase tracking-wider ${spotifyData.isPlaying ? 'text-[#1DB954]' : 'text-slate-400'}`}>{spotifyData.isPlaying ? 'Now Playing' : 'Spotify Offline'}</span>
                </div>
                <p className="text-slate-900 font-bold text-xs sm:text-sm truncate leading-tight">{spotifyData.song}</p>
                <p className="text-slate-500 text-[10px] sm:text-xs truncate">{spotifyData.artist}</p>
              </div>
            </div>
          </TiltCard>
          ...
          <div className="col-span-1 md:col-span-1 lg:col-span-1 flex flex-col gap-4 sm:gap-5 h-full w-full">
            <TiltCard initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }} className="flex-1 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-[2rem] p-5 sm:p-6 text-white flex flex-col justify-between group shadow-lg shadow-blue-500/20 border border-blue-400/30 transition-transform relative overflow-hidden min-h-[120px] sm:min-h-0 h-full w-full block">
              ...
              <div className="flex justify-between items-start relative z-10 w-full pointer-events-none">
                <h3 className="font-syne text-lg sm:text-xl font-bold leading-tight">Deployments &<br />Case Studies</h3>
            ...
            <TiltCard initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.6 }} className="h-[60px] sm:h-[72px] shrink-0 bg-white/80 backdrop-blur-3xl rounded-[2rem] p-4 sm:p-5 border-2 border-white/80 text-slate-800 font-bold flex items-center justify-center gap-2 hover:bg-white transition-colors shadow-sm w-full block relative">
              <a href="/resume.pdf" download className="absolute inset-0 w-full h-full z-20"></a>
              <div className="flex items-center justify-center gap-2 pointer-events-none text-sm sm:text-base"><FileText size={18} className="text-slate-500" /> Download CV</div>
            </TiltCard>
          </div>
      ...
      <section className="relative z-20 py-8">
        <div className="container mx-auto px-4 sm:px-6 md:px-10 lg:px-16 max-w-7xl">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            {[
              { title: "Backend Magic 🔮", icon: <Database className="text-cyan-500 w-8 h-8 sm:w-10 sm:h-10 mb-3 sm:mb-4" />, desc: "Python, Firebase, Node.js" },
              { title: "Pixel Perfect UI ✨", icon: <LayoutTemplate className="text-indigo-500 w-8 h-8 sm:w-10 sm:h-10 mb-3 sm:mb-4" />, desc: "Next.js, TypeScript, Figma" },
              { title: "AI Innovation 🤖", icon: <Code2 className="text-blue-500 w-8 h-8 sm:w-10 sm:h-10 mb-3 sm:mb-4" />, desc: "Generative AI, Prompt Engineering" }
            ].map((skill, i) => (
              <TiltCard key={i} {...sectionReveal} transition={{ delay: i * 0.1, duration: 0.5 }} className="p-6 sm:p-8 rounded-[2rem] bg-white/60 backdrop-blur-xl border-2 border-white/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] group cursor-default">
                {skill.icon}
                <h3 className="font-syne text-lg sm:text-xl font-bold mb-2 text-slate-900 group-hover:text-blue-600 transition-colors pointer-events-none">{skill.title}</h3>
                <p className="text-slate-600 text-sm sm:text-md font-sans pointer-events-none">{skill.desc}</p>
              </TiltCard>
            ))}
          </div>
      ...
      <section className="relative z-20 py-8">
        <div className="container mx-auto px-4 sm:px-6 md:px-10 lg:px-16 max-w-7xl">
          <TiltCard {...sectionReveal} className="bg-white/60 backdrop-blur-xl rounded-[2.5rem] p-6 sm:p-10 lg:p-16 border-2 border-white/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden">
            <div className="absolute -left-32 -top-32 w-96 h-96 bg-blue-100/50 rounded-full blur-[80px] pointer-events-none"></div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-14 items-center relative z-10">
              <div className="flex flex-col justify-center text-left items-start">
                ...
                <h2 className="font-syne text-3xl sm:text-4xl lg:text-5xl font-bold text-slate-900 mb-4 sm:mb-6 leading-tight tracking-tight">
                  Engineering student by day,<br className="hidden sm:block" /> creative developer by night.
                </h2>
                <p className="text-sm sm:text-base lg:text-lg text-slate-600 font-sans mb-6 sm:mb-8 leading-relaxed">
                ...
                <Link href="/about" className="px-6 sm:px-8 py-3 sm:py-4 bg-slate-900 text-white font-bold rounded-2xl hover:bg-slate-800 transition-all inline-flex items-center gap-2 shadow-lg hover:shadow-xl hover:-translate-y-1 text-sm sm:text-base w-full sm:w-auto justify-center relative z-20">
                  Read Full Story
                </Link>
              </div>

              <div className="relative min-h-[300px] md:min-h-[350px] lg:min-h-[400px] w-full rounded-[2rem] flex flex-col overflow-hidden group bg-white shadow-xl border border-slate-200 pointer-events-none">
                ...
                <div className="flex-1 w-full h-full bg-transparent flex flex-col overflow-hidden relative z-10">
                  <div className="h-10 md:h-12 bg-slate-50/80 backdrop-blur-md border-b border-slate-200 flex items-center px-4 md:px-6 gap-1.5 sm:gap-2 shrink-0">
                    ...
                    <span className="mx-auto text-[10px] sm:text-xs md:text-sm text-slate-500 font-mono flex items-center gap-2">
                      <Terminal size={14} className="text-slate-400 hidden sm:block" /> developer.py
                    </span>
                  </div>

                  <div className="p-4 sm:p-6 md:p-8 font-mono text-[10px] sm:text-xs md:text-sm lg:text-base leading-loose overflow-x-auto text-slate-700 relative z-10 flex flex-col justify-center h-full w-full">
      ...
      <section className="relative z-20 py-8">
        <div className="container mx-auto px-4 sm:px-6 md:px-10 lg:px-16 max-w-7xl">
          <motion.div {...sectionReveal} className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 sm:mb-8 gap-4 sm:gap-6">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-blue-100/50 border border-blue-200 text-blue-700 font-bold text-[10px] sm:text-sm mb-3 sm:mb-4 shadow-sm">
                <Star size={14} className="fill-blue-700" /> FEATURED WORK
              </div>
              <h2 className="font-syne text-3xl sm:text-4xl font-bold text-slate-900">Top Projects.</h2>
            </div>
            <Link href="/projects" className="px-6 py-3 bg-white/60 backdrop-blur-xl border-2 border-white/80 text-slate-900 font-bold rounded-2xl hover:bg-white transition-colors flex items-center justify-center gap-2 shadow-sm hover:scale-[1.02] w-full md:w-auto text-sm sm:text-base">
          ...
          {featuredProjects.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
              {featuredProjects.map((project, i) => (
                <TiltCard key={project.id} {...sectionReveal} transition={{ delay: i * 0.1 }} className="group bg-white/80 backdrop-blur-xl rounded-[2rem] border-2 border-white/80 overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] flex flex-col cursor-pointer">
                  <div className="h-8 sm:h-10 bg-slate-100/80 backdrop-blur-md flex items-center px-4 sm:px-6 gap-1.5 sm:gap-2 border-b border-slate-200 relative">
                    ...
                    <div className="mx-auto text-[10px] sm:text-xs font-mono text-slate-500">project.ts</div>
                  </div>

                  <div className="h-40 sm:h-48 lg:h-56 bg-slate-50 p-6 sm:p-8 flex items-end relative overflow-hidden">
                    ...
                    {project.imageUrl && (
                      <Image src={project.imageUrl} alt={project.title} fill className="absolute inset-0 object-cover opacity-20 mix-blend-multiply group-hover:scale-105 transition-transform duration-700" />
                    )}
                    <h3 className="font-syne text-2xl sm:text-3xl font-bold text-slate-900 relative z-10 truncate w-full pointer-events-none">{project.title}</h3>
                  </div>

                  <div className="p-6 sm:p-8 bg-transparent flex-1 flex flex-col border-t border-slate-100 pointer-events-none">
                    <p className="text-slate-600 mb-4 sm:mb-6 line-clamp-2 text-xs sm:text-sm leading-relaxed">{project.description}</p>
                    <div className="flex gap-2 flex-wrap mt-auto">
                      {(project.tags || []).slice(0, 3).map((t: string) => (
                        <span key={t} className="px-2.5 py-1 sm:px-3 sm:py-1.5 bg-slate-100 text-slate-700 rounded-lg text-[10px] sm:text-xs font-bold border border-slate-200">{t}</span>
                      ))}
                    </div>
                  </div>
                </TiltCard>
      ...
      <section className="relative z-20 py-8">
        <div className="container mx-auto px-4 sm:px-6 md:px-10 lg:px-16 max-w-7xl">
          <TiltCard {...sectionReveal} className="bg-white/60 backdrop-blur-xl rounded-[2.5rem] p-6 sm:p-10 lg:p-14 border-2 border-white/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 sm:mb-10 gap-4 sm:gap-6">
              <div className="text-center md:text-left pointer-events-none">
                <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-orange-100/50 border border-orange-200 text-orange-700 font-bold text-[10px] sm:text-sm mb-3 sm:mb-4">
                  <Star size={14} className="fill-orange-700" /> FEATURED CERTIFICATES
                </div>
                <h2 className="font-syne text-3xl md:text-4xl font-bold text-slate-900">Continuous Learning.</h2>
              </div>
              <Link href="/certifications" className="relative z-20 px-6 sm:px-8 py-3 sm:py-4 bg-slate-900 text-white font-bold rounded-2xl hover:bg-slate-800 transition-colors shadow-lg text-sm sm:text-base w-full md:w-auto text-center">
                View Full Vault
              </Link>
            </div>
            ...
            {featuredCerts.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 pointer-events-none">
                {featuredCerts.map((cert) => (
                  <div key={cert.id} className="p-4 sm:p-6 bg-white rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4 sm:gap-5 transition-all">
                    ...
                    <div className="overflow-hidden">
                      <h3 className="font-bold text-base sm:text-lg text-slate-900 truncate">{cert.title}</h3>
                      <p className="text-slate-500 text-xs sm:text-sm font-medium">{cert.issuer}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="relative z-20 py-8 mb-6 sm:mb-12">
        <div className="container mx-auto px-4 sm:px-6 md:px-10 lg:px-16 max-w-7xl">
          <motion.div {...sectionReveal} className="bg-slate-900 text-white rounded-[2.5rem] sm:rounded-[3rem] p-8 sm:p-10 md:p-16 lg:p-20 text-center shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-3xl mx-auto">
              <h2 className="font-syne text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-4 sm:mb-6 leading-tight">Ready to collaborate?</h2>
              <p className="text-slate-400 text-sm sm:text-base md:text-lg mb-8 sm:mb-10 px-4 sm:px-0">
                Whether it is a scalable backend architecture, a sleek UI/UX design, or deploying the next big idea. I'm ready to build.
              </p>
              <Link href="/contact" className="px-6 sm:px-8 lg:px-10 py-3 sm:py-4 lg:py-5 bg-white text-slate-900 font-bold rounded-2xl hover:scale-105 transition-all text-sm sm:text-base lg:text-lg inline-flex items-center justify-center gap-3 shadow-xl w-full sm:w-auto">
                <Send size={18} className="sm:w-5 sm:h-5" /> Say Hello
              </Link>
            </div>
          </motion.div>
        </div>
