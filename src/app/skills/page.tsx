  ...
  return (
    <div className="container mx-auto px-6 md:px-12 py-24 min-h-screen relative z-10 bg-background">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-4xl mb-16">
        ...
        <p className="text-lg md:text-xl text-slate-700 font-sans max-w-2xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          {Object.entries(groupedSkills).map(([category, categorySkills], index) => (
            <motion.div 
              ...
              className="bg-white p-8 md:p-10 rounded-3xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow"
            >
              <h2 className="font-syne text-2xl font-bold text-slate-900 mb-8 pb-4 border-b border-slate-100">{category}</h2>
              <div className="space-y-6">
                {categorySkills.map((skill, i) => (
                  <div key={skill.id}>
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-bold text-slate-800">{skill.name}</span>
                      <span className="font-mono text-sm font-bold text-accent">{skill.level}%</span>
                    </div>
