...
export default function Navbar() {
  ...
  return (
    <>
      ...
      {/* --- MOBILE NAV: SLEEK FLOATING PILL --- */}
      <motion.nav 
        initial={{ y: 0 }}
        animate={{ y: showMobileNav ? 0 : -100 }}
        transition={{ duration: 0.3, ease: "easeInOut" }}
        className="fixed top-4 inset-x-4 z-50 md:hidden bg-white/70 backdrop-blur-2xl border border-white/50 shadow-[0_8px_30px_rgb(0,0,0,0.08)] rounded-[2rem] px-5 py-3 flex items-center justify-between pointer-events-auto"
      >
        <Link href="/" className="font-syne font-black text-xl sm:text-2xl tracking-tighter text-slate-900">
           ARPAN<span className="text-blue-600">.</span>
        </Link>
        <button 
          onClick={() => setIsMobileMenuOpen(true)} 
          className="p-2 sm:p-2.5 text-slate-700 bg-white/80 hover:bg-white rounded-full border border-slate-200 shadow-sm transition-all active:scale-95 flex items-center justify-center"
        >
          <Menu size={18} />
        </button>
      </motion.nav>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div 
            ...
            className="fixed inset-0 z-[100] bg-white/95 backdrop-blur-3xl flex flex-col pointer-events-auto p-6 md:hidden overflow-hidden max-w-[100vw]"
          >
            <div className="flex justify-center items-center pt-2 pb-4 opacity-30 pointer-events-none">
              ...
              {allLinks.map((link, i) => (
                ...
                >
                  <Link
                    href={link.path}
                    onClick={() => setIsMobileMenuOpen(false)}
                    className={`block py-4 font-syne font-extrabold text-4xl tracking-tight transition-colors border-b border-slate-100 ${
                      isActive(link.path) ? "text-blue-600" : "text-slate-800 hover:text-blue-500"
                    }`}
                  >
                    {link.name}
                  </Link>
